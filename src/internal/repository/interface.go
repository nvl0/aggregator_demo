...
type Channel interface {
	LoadChannelList(ctx context.Context, ts transaction.Session) (channelList []channel.Channel, err error)
}

type Session interface {
	LoadOnlineSessionList(ctx context.Context, ts transaction.Session) (sessList []session.OnlineSession, err error)
	SaveChunkList(ctx context.Context, ts transaction.Session, chunkList []session.Chunk) error
}
...
type FlowBatch interface {
	...
	LoadCommittedFileNames(ctx context.Context, ts transaction.Session, nasIP string) (
		fileNameSet map[string]bool, err error)
	// SaveFileNames запись имен flow файлов, повторная запись известного имени не ошибка
	SaveFileNames(ctx context.Context, ts transaction.Session, nasIP string, fileNameList []string) error
	// RemoveByNasIP удаление всех записей чекпоинта по nas_ip
	RemoveByNasIP(ctx context.Context, ts transaction.Session, nasIP string) error
}
